...
test("visual system implements light and night responsive shells", () => {
  ...
  assert.match(cssSource, /\.bottom-nav\s*\{[\s\S]*position:\s*fixed/u);
  assert.match(cssSource, /@media \(min-width:\s*760px\)[\s\S]*\.app-shell\s*\{[\s\S]*grid-template-columns:\s*220px minmax\(0,\s*1fr\)/u);
...
test("run screen keeps question options, break moves, and submit controls adjacent", () => {
  ...
  assert.match(body, /lesson-strip[\s\S]*question-panel[\s\S]*option-grid[\s\S]*move-panel[\s\S]*run-actions/u);
